...
0. 데이터 불러오기
```{r}
directory <- "C:/Users/USER/Desktop/압축파일"
...
library(doBy)
summaryBy(.~income, data = adult) # 디폴트는 평균, 다양하게 사용 가능
```
...
```{r}
colSums(is.na(adult)) # 결측치가 어느 열에서 나왔는지
```
...
3-2. 데이터 통합
```{r}
edu <- read.csv(paste0(directory, "\\edu.csv")) # education을 수치화한 데이터
View(edu)
```
...
{
  wh <- which(edu$education == adult1$education[i]) # 같은곳을 찾고
...
```{r}
shapiro.test(sample(adult$fnlwgt, 5000)) # shaprio.test : 정규분포 인가?
```
...
```{r}
test2 <- sampleBy(~ income, frac = 0.3, data = adult5) # 분포를 동일하게 나눔
